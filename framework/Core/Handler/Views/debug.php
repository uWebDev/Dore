...
<html>
    <head>
        ...
        <style>
            ...
            body{
                ...
                min-height:100%;
                background: #8d0909; /* Old browsers */
                background: -moz-radial-gradient(center, ellipse cover, #8d0909 0%, #8d0909 100%); /* FF3.6+ */
                background: -webkit-radial-gradient(center, ellipse cover, #8d0909 0%,#8d0909 100%); /* Chrome10+,Safari5.1+ */
            }
            #content{
                max-width:80%;
                min-width:1000px;
    ...
    <body>
        ...
        ?>
        <div id="content">
            <div id="exception"><?= str_replace("\n", '<br/>', $exception->getMessage()); ?></div>
            <hr>
            <hr>
            <div id="blocks">
                <?php foreach ($blocks as $bkey => $block): ?>
                    <div class="block <?= (!empty($log) && $bkey == 0) ? 'thick' : ''; ?>">
                        ...
                        <div class="code"><?php
                            ...
                            <?php foreach ($code as $n => $text): ?>
                                <pre class="line <?= $n == $line ? 'highlight' : ''; ?>"><?= ($n + 1) . '    ' . htmlspecialchars($text); ?></pre>
                            <?php endforeach; ?>
                        </div>
                    </div>
