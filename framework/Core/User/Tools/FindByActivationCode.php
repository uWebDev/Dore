...
class FindByActivationCode
{
    ...
    public static function find($code, \PDO $db)
    {
        try {
            ...
            $stmt = $db->prepare("SELECT * FROM `users` WHERE `activationHash` = :code AND `provider` = 'native' LIMIT 1");
    ...
    private static function checkResult($result, $code)
    {
        if (false === $result || !is_array($result)) {
            throw new UserNotFoundException(_s('The link to activate your account is not valid'));
        }
    }
